const { lupin } = require('../framework/lupin');
...
lupin({nomCom:"bot",reaction:"��",categorie:"IA"},async(dest,zk,commandeOptions)=>{
...
lupin({ nomCom: "dalle", reaction: "📡", categorie: "IA" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    let caption = '*Propulsé par LUPIN-MD*';
...
lupin({ nomCom: "gpt", reaction: "", categorie: "IA" }, async (dest, zk, commandeOptions) => {
  const { repondre, arg, ms } = commandeOptions;
